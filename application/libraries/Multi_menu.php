...
class Multi_menu {
	 ...
	 * @var string
	 */
	private $item_anchor              = '<a class="%s" href="%s">%s</a>';
	...
	{	    
	    ...
	    foreach ($items as $item)
	    {
	        ...
	        if ( isset($item[$this->menu_label], $item[$this->menu_key]) ) 
	        {
				...
				#disini htl setactive	        	        

				if (substr_count($item_anchor, '%s') == 3) {
					$html .= sprintf($item_anchor, $this->set_active_anchor($tag_open, $slug), $href, $label);
				}
				else {
	...
	private function set_active_anchor($html, $slug)
	{
		...
		}elseif ($segment1!='' && $segment2!='' && $segment3!='') {
